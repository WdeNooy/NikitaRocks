...
# analyses of loudness at the event level with several exposure variables.
...
# 4. expo_minute_last_max_playmate: The loudest voice of playmates' last utterances in the preceding minute. 
# 5. expo_minute_last_max_conversation: The loudest voice of conversation partners' last utterances in the preceding minute. 
# 6. expo_minute_last_max_other: The loudest voice of all other pupils' last utterances in the preceding minute. 
loudness_events <- cases_help %>%
  ...
  summarise(
    expo_minute_last_max_playmate = max(loudness_playmate, na.rm = TRUE),
    expo_minute_last_max_conversation = max(loudness_conversation, na.rm = TRUE),
    expo_minute_last_max_other = max(loudness_other, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  #replace -Inf by NA
  mutate(
    expo_minute_last_max_playmate = ifelse(
      is.infinite(expo_minute_last_max_playmate),
      NA, expo_minute_last_max_playmate),
    expo_minute_last_max_conversation = ifelse(
      is.infinite(expo_minute_last_max_conversation),
      NA, expo_minute_last_max_conversation),
    expo_minute_last_max_other = ifelse(
      is.infinite(expo_minute_last_max_other),
      NA, expo_minute_last_max_other)
  ) %>%
  #replace NA by variable mean
  mutate(
    expo_minute_last_max_playmate = ifelse(
      is.na(expo_minute_last_max_playmate),
      mean(expo_minute_last_max_playmate, na.rm = TRUE),
      expo_minute_last_max_playmate),
    expo_minute_last_max_conversation = ifelse(
      is.na(expo_minute_last_max_conversation),
      mean(expo_minute_last_max_conversation, na.rm = TRUE),
      expo_minute_last_max_conversation),
    expo_minute_last_max_other = ifelse(
      is.na(expo_minute_last_max_other),
      mean(expo_minute_last_max_other, na.rm = TRUE),
      expo_minute_last_max_other)
  )
...
A pupil can be loud at some moments and not so loud on other moments during a break. So let us not analyze average loudness of a pupil over the entire break. Instead, let us analyze all events that pupils raise their voices.
...
Truyen is playing with Shrika and Desiree, both of whom are also talking. Truyen is talking with Desiree in a friendly tone (mutual green arrows), whereas Shrika seems to be having an argument with Hunter (mutual red arrows).
...
2. also on preceding loudness of other pupils,
...
For each time a pupil uses their voice, we need one and only one score that reflects the exposure to other peers' voices.
...
Note that the last utterance does not necessarily yield one exposure score. In our example, 5 pupils are still talking at time 2:12. If this can happen, we must combine selection of the last utterance of a pupil with taking the mean, maximum, or minimum.
...
Example: Truyen's exposure as the average value of the loudness of each pupil's last utterance in the preceding minute.
...
The dataset *loudness_events* contains several exposure variables:
...
1. Estimate @. Compare @ (because all exposure variables are on the same scale (from 0 to 1), we can compare the regression coefficients). Try to formulate why you think one type of exposure has a stronger effect (higher regression coefficient) than another type of exposure.
</div>
