...
### The pair as observation

In this session, we will use the data frame `utterances` in our analyses. Have a look at the part of this data set that represents the utterances depicted in the previous figure (pupil IDs are replaced by their first names; onset and terminus times in minutes and seconds).
...
utterances %>%
  ...
  #change onset and temrinus into time character strings
  mutate(
    onset = paste0(format(round(onset), width = 2), ":",
                   formatC(round(60 * (onset %% 1)),
                           flag = "0", width=2)),
    terminus = paste0(format(round(terminus), width = 2), ":",
                   formatC(round(60 * (terminus %% 1)),
                           flag = "0", width=2))
  ) %>%
...
A tie is equal to a pair of nodes. For lines, the order of nodes (from versus to) does not matter, but the order matters for arrows (arcs). Instead of analyzing characteristics of nodes (pupils), we are now going to analyze a characteristic of pairs of nodes. In our example, whether a statement is positive or negative.
...
1. Which of the types of independent/predictor variables do you recognize in the above data? Tips: Use the little black triangle to see more variables and read the description of the variables (may open in new browser window).

```{r datadescription, exercise = TRUE}
?utterances
```

2. Can you figure out how the variable `simil_sex` was constructed?
</div>
...
Predictors for effects of other relations (friendship, playmate) include the Yes(1)/No(0) variable used to register friendships and playmates in the basic data sets for pairwise (= lines) information: `pairs_const` and `pairs_dyn`.
...
In the data set that we will use (`utterances`), only the last statement between each pair of pupils in the last 5 minutes (before the new statement) has been used to calculate two balance predictor variables:
...
predict(
  ...
  data.frame( #enter the variables and values for which you want predictions:
    sex_from = c(0, 1), #zero for boys, one for girls
    adhd_to = c(1.13, 1.13), #both the addressed boy and girl have average ADHD level...
    simil_ethn = c(1, 1), #speaker and addressee have the same ethnicity
    friend = c(0, 0), #but they are not friends
    received_neg = c(0, 0), #the speaker did not receive previous negative statements from the addressee
...
```{r logistic2, exercise = TRUE, exercise.timelimit = 180}
...
The random (varying) intercepts for the sender of the statement (`from`) represent pupils' personal tendencies to make negative rather than positive statements.

The random (varying) intercepts for the receiver of the statement (`to`) represent pupils' personal tendencies to be the target of negative rather than positive statements.

```{r multilevelplot2, exercise = TRUE, exercise.eval = TRUE}
# Estimate the cross-nested multilevel model.
model2 <- glmer(
  negative ~ sex_from + adhd_to + simil_ethn + friend + received_neg + balance_neg + (1 | from) + (1 | to),
  data = utterances,
  family = binomial(link = "logit")
...
# Helper file: random intercepts for sender (from)
from <- ranef(model2, drop = TRUE)$from %>% as_tibble(rownames = "ID")
# Helper file: random intercepts for receiver (to)
to <- ranef(model2, drop = TRUE)$to %>% as_tibble(rownames = "ID")
# Combine files and create plot.
from %>%
  #add receiver intercepts
  full_join(to, by = "ID") %>%
  ...
  geom_hline(yintercept = 0, colour = "blue") +
    ...
    #add black line segments for varying sender intercepts
    #with pupil labels (on X axis) ordered by exposure effect
    geom_errorbar(aes(x = reorder(label, value.x), ymin = value.x, ymax = value.x)) +
    #add red line segments for varying exposure effects
    geom_errorbar(aes(x = reorder(label, value.y), ymin = value.y, ymax = value.y), colour = "Red") +
    #changing the labels of the axes
    labs(x = "", 
         y = "Estimates",
         title = "Varying intercepts for senders (black) and receivers (red)"
         ) +
...
1. Which pupils are most likely to make negative statements, which ones are least likely?

2. Which pupils are most likely to attract negative statements, which ones are least likely?
</div>
