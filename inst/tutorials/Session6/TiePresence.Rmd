...
# COPY THIS COMMAND TO THE RStudio CONSOLE AND RUN IT THERE. 

# If you haven't installed the goldfish package yet, use the below command to
...
The `goldfish` package has functions that create predictor variables for sender, receiver, (dis)similarity, other tie, same tie, and local network context effects. That saves us a lot of work!
...
1. Why do some effects require a network and others only an attribute?
...
For example, pupils who are one unit more similar to the speaker (sender) are around `r round(exp(model_statement_choice$parameters[[1]]))` more likely to be selected as target (if they are not a playmate of the speaker and did not make statements to the speaker in the preceding 3 minutes).
...
As a consequence, we can use all types of effects from the `goldfish` package. For effects that may address either the sender or receiver, we must use the `type = "ego"` or `type = "alter"` argument to clarify which of the two we mean:
...
1. A DyNAM model uses reciprocity to model only the statement target, not the speaker. In contrast, a REM may use reciprocity both to select the speaker and target (because it selects the arrow). Which model is intuitively more plausible to you?
...
2. Run the code (may take some time; alternative: only run the `summary()` function) and inspect the output (in the last output box). Interpret the signs (directions) of the coefficients: What increases the probability that two pupils start playing together, what decreases the probability?
...
Because of the guessing, `RSiena` requires that the ties do not change too much between subsequent panel waves. A substantial proportion of ties that exist in a panel wave must still exist in the next. 
