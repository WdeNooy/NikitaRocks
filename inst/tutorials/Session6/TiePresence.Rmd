...
# COPY THIS COMMANDS TO THE RStudio CONSOLE AND RUN IT THERE. This will install the goldfish package from github.
...
2. Run the above code. This should open new browser window with general info on the package.
</div>
...
For example, pupils who are one unit more similar to the speaker (sender) are around `r round(exp(coef(model_statement_choice)[[1]]))` more likely to be selected as target (if they are not playmate of the speaker and did not make statements to the speaker in the preceding 3 minutes).
